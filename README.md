**Curso completo de Java Script**

**AULA 25:** 
Nesta aula é feito alguns exercicíos utilizando strings, para aprofundar um pouco mais o conhecimento sobre STRINGS.

**AULA 26:** 
Nesta aula aprendi mais sobre os Numbers, funções para poder resolver problemas ou para modelar os valores.

**AULA 27:** 
Aula sobre Math e numbers, para reforçar os conhecimentos sobre numeros e aprender sobre o onjeto Math.

**PRATICA 01:**
Fiz um teste prático para reforçcar oq aprendi ate agora, fazendo uma pequena calculadora de IMC, onde utilizei auxilio do ChatGpt e documentações para corrigir meus erros. Com essa prática reforcei meu aprendizado em Number e manipulação do HTML.

**PRATICA 02:** 
Fiz um teste prático para reforçcar oq aprendi ate agora, resolvendo um pequeno problema de logica e comportamento, onde utilizei auxilio de documentações para corrigir esses erros. Com essa prática reforcei meu aprendizado em Number, String, logica e manipulação do HTML.

**AULA 34:**
Foi feito um teste prático, onde foi utilizado o aprendizado em function, arrays e objects. Nesse teste é coletado os dados do html e armazenado em um array que após enviar o formulario, os dados são apresentados no html.

**Curso completo de Java Script - Logica de Programação**

**Aula 40:**
Aula de logica de programação sobre if, else if e else, junto tambem com alguns operadores logicos e de comparação.

**Aula 44:**
Aula com um teste pratico, com um mini projeto para calcular o imc e dizer como está sua saúde (abaixo do peso, normal, sobrepeso e etc). Nesse mini projeto foi utilizado tudo que foi aprendido no modulo de logica, operadores logicos, operadores de comparação, if, else if, else, function.

**Aula 44:**
Aula ensinando Switch/case, onde tem semelhancia com if/else if. Utilizado para fazer verificações de valores de uma variavel.

**JS Logica de Programação/Pratica 1:** 
Este projeto tem como objetivo simular o funcionamento básico de um sistema de pedidos em uma loja virtual, desenvolvido inteiramente em JavaScript. Nele, é possível escolher um produto, definir uma quantidade e obter o valor final com desconto, de acordo com a categoria do item selecionado.

O sistema utiliza um array de objetos para armazenar os produtos disponíveis, cada um contendo nome, preço, categoria e estoque. A função principal, fazerPedido(produtoEscolhido, quantidade), é responsável por localizar o produto dentro do array, verificar se ele existe e se há estoque suficiente. Caso o produto seja encontrado, o programa aplica um desconto conforme sua categoria — utilizando a estrutura switch/case — e retorna um objeto com os dados do pedido, incluindo o nome do produto, a quantidade e o preço final com desconto.

Durante o desenvolvimento, alguns ajustes foram necessários. O principal foi corrigir um erro de digitação no nome de um produto, o que impedia sua identificação. Também foi corrigido o uso incorreto do switch para validar a quantidade (que comparava valores de forma errada) e a falta de uma verificação que impedia o acesso a propriedades de um produto inexistente.

Foram aplicados diversos conceitos importantes, como funções, estruturas condicionais (if/else), método .find(), arrays, objetos e operações matemáticas básicas para cálculo de desconto.

Esse projeto é um ótimo exemplo de como combinar lógica de programação, organização de dados e estruturas de decisão em um único código funcional e bem estruturado.

**JS Logica de Programação/Pratica 2:**
Este projeto tem como objetivo praticar atribuição via desestruturação em objetos e arrays aninhados.
O desafio simula uma resposta de API real, retornando dados de um cliente e seus pedidos, como seria em um backend com Node.js puro.

A missão é extrair, de forma limpa e direta, apenas as informações necessárias, sem acessar manualmente cada nível do objeto.